...
-- Alejandro Arevalo Merlo 1.
select * from productos order by precio desc;

-- Alejandro Arevalo Merlo 2.
select * from pedidos where estado = 'pendiente';

-- Alejandro Arevalo Merlo 3.
select count(Id_producto) as Productos_caros from productos where precio > 500.00;

-- Alejandro Arevalo Merlo 4.
select Nombre from clientes where substr(Telefono, 8, 9) = 56;

-- Alejandro Arevalo Merlo 5.
select distinct Estado from pedidos;

-- Alejandro Arevalo Mero 6. A
select * from pedidos where sysdate - 2023-01; 

-- Alejandro Arevalo Merlo 7.
select Id_cliente, nombre, UPPER(concat(direccion, '-', ciudad)) as UBICACION from clientes;

-- Alejandro Arevalo Merlo 8. 
select Fecha_pedido from pedidos order by Fecha_pedido asc limit 1;

-- Alejandro Arevalo Merlo 9. 
select Pais, count(Id_fab) as fabricantes_por_pais from fabricantes group by Pais; 

-- Alejandro Arevalo Merlo 10.
select * from productos where precio between 200 and 800;

-- Alejandro Arevalo Merlo 11.
select * from productos order by precio desc limit 1;

-- Alejandro Arevalo Merlo 12.
select round(precio, 1) as Precio_redondeado from productos ;

-- Alejandro Arevalo Merlo 13. A
select * from fabricantes;
select * from productos;
select Id_fab, count(Existencias) from productos where Existencias >= 30 group by Id_fab;

-- Alejandro Arevalo Merlo 14.  A
-- select * from productos;
  select Existencias
	 case
	 when Existencias >= 30 then Estado = 'muchas'
        when Existencias < 10 then Estado = 'pocas'
        else as Estado = 'suficientes'
        end;
        
-- Alejandro Arevalo Merlo 15. 
select Id_categoria, sum(Existencias) as suma from productos group by Id_categoria having sum(Existencias) >= 50;

-- Alejandro Arevalo Merlo 16.
select * from pedidos;
select distinct Metodo_pago, count(N_pedido) as cantidad_pedidos from pedidos group by Metodo_pago;
